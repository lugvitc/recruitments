...
function Questions({ dep, name }: {[key: string]: string}) {
    return (
        <>
            {
                questions[dep] !== undefined ? questions[dep]!.map(
                    (question, index) => {
                        return (
                            <Fragment key={index}>
                                <label htmlFor={`${name}.${index}`}>
...
function Preference({ req, ...props }: any) {
    ...
    return (
        <Field as="select" {...props} defaultValue="none">
            <option disabled={req === true} value="none" style={req === true ? {display: "none"} : {}}>None</option>
            {
...
function Application({ values, isSubmitting, isValidating}: any) {
    ...
    return (
        <Form className='flex flex-col gap-2 p-8' autoComplete="off">
                                        ...
                                        (app: any, index: number) => (
                                            <div key={index} className='flex flex-col gap-2'>
                                                <label htmlFor={`apps.${index}.dep`}>
                                                    Department Preference {index + 1}<br />{pref[index]}
                                                </label>
                                ...
                                values.apps.length + count < 2 && (
                                    <>
                                        <label htmlFor='temppref'>
                                            <h2>Department Preference {values.apps.length + 1}</h2> {pref[values.apps.length]}
                                        </label>
                                        <Preference key={values.apps.length} name="temppref" onChange={(sel: any) => { arrayHelpers.push({dep: sel.target.value, questions: []});}} req={values.apps.length !== 1} />
                                    </>
                                )
                            }
                            {
                                count === 1 ? (
                                    <div className=' text-green-300 italic'>You have successfully submitted one preference till now.</div>
                                ) : null
                            }
                        </div>
                    )
            ...
            <button
                type="submit"
                className={ "rounded-md mt-4 text-center p-2 text-black font-semibold transition-all border max-w-28" + (
                    isSubmitting && !isValidating ? " bg-yellow-700 border-yellow-700" : " bg-yellow-400 hover:bg-black hover:text-white border-yellow-400"
                )}
...
function validateForm(values: any) {
    const errors: any = {};
    if (values.apps.length === 0) errors.apps = "Please fill atleast one department preference";
    else if (values.apps.length === 2 && values.apps[0].dep === values.apps[1].dep) errors.apps = "Second preference cannot be same as first preference."
    return errors;
}

function onSumbitFactory(sig: number, setSig: (arg0: number) => any, session: any) {
    async function onSubmit(values: any, formikBag: any) {
        ...
        const rows = values.apps.filter(
            (app: any) => app.dep !== "none" && app.dep !== undefined && app.dep !== null
        ).map(
            (app: any) => (
                {
                    ...common,
                    dep: app.dep,
                    formdata: {
                        questions: Object.fromEntries(questions[app.dep]!.map((q, i) => [q, app.questions[i]])),
                        common_questions: Object.fromEntries(common_questions.map((q, i) => [q, values.common_questions[i]]))
                    }
                }
            )
        )
...
export default function Apply() {
    ...
    useEffect(() => {
        ...
        supabase
        .from('applicants')
        .select('email').eq("email", session!.user!.email!).then((records) => {setCount(records.data?.length)})
                ...
                <CountContext.Provider value={count || 0}>
                    {
                        ...
                        (
                            <div><span className='text-2xl'>Not logged in</span></div>
                        )
                    }
